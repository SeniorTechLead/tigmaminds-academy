...
const LEAF_NORMAL = 'fill-gray-200 dark:fill-slate-800';
const LEAF_STROKE_NORMAL = 'stroke-gray-400 dark:stroke-gray-600';
  ...
  return (
    <div className="w-full max-w-lg mx-auto my-4">
      <div className="flex gap-2 mb-2 justify-center">
        <button
          ...
          className={`px-3 py-1 rounded text-xs font-medium transition-colors ${
            round === 1 ? 'bg-blue-600 text-white' : INACTIVE_BTN
          }`}
        >
          Round 1 (prior 0.1%)
        </button>
        <button
          ...
          className={`px-3 py-1 rounded text-xs font-medium transition-colors ${
            round === 2 ? 'bg-blue-600 text-white' : INACTIVE_BTN
          }`}
        >
          Round 2 (prior 9%)
        </button>
        <button
          ...
          className={`px-3 py-1 rounded text-xs font-medium transition-colors ${
            highlighted ? 'bg-amber-600 text-white' : INACTIVE_BTN
          }`}
        >
      ...
      >
        ...
        <text x={(level1X + level2X) / 2 + 5} y={tpY - 3} className={TEXT_MID} fontSize="7" fontFamily="system-ui, sans-serif">99%</text>

        {/* Disease -> False Negative */}
        <line x1={level1X + 30} y1={diseaseY + 5} x2={level2X - 25} y2={fnY} className={LEAF_STROKE_NORMAL} strokeWidth="1" />
        <text x={(level1X + level2X) / 2 + 5} y={fnY + 10} className={TEXT_SUBTLE} fontSize="7" fontFamily="system-ui, sans-serif">1%</text>
        ...
        <text x={(level1X + level2X) / 2 + 5} y={fpY - 3} className={TEXT_MID} fontSize="7" fontFamily="system-ui, sans-serif">1%</text>

        {/* No Disease -> True Negative */}
        <line x1={level1X + 30} y1={noDiseaseY + 5} x2={level2X - 25} y2={tnY} className={LEAF_STROKE_NORMAL} strokeWidth="1" />
        <text x={(level1X + level2X) / 2 + 5} y={tnY + 10} className={TEXT_SUBTLE} fontSize="7" fontFamily="system-ui, sans-serif">99%</text>

        {/* Leaf: True Positive */}
        <rect x={level2X - 25} y={tpY - 10} width="60" height="20" rx="3"
          className={highlighted ? '' : `${LEAF_NORMAL} ${LEAF_STROKE_NORMAL}`}
          fill={highlighted ? '#78350f' : undefined} stroke={highlighted ? '#f59e0b' : undefined} strokeWidth="1" />
        ...
        {/* Leaf: False Negative */}
        <rect x={level2X - 25} y={fnY - 10} width="60" height="20" rx="3" className={`${LEAF_NORMAL} ${LEAF_STROKE_NORMAL}`} strokeWidth="1" />
        <text x={level2X + 5} y={fnY + 1} textAnchor="middle" className={TEXT_SUBTLE} fontSize="7" fontFamily="system-ui, sans-serif">
        ...
        {/* Leaf: True Negative */}
        <rect x={level2X - 25} y={tnY - 10} width="60" height="20" rx="3" className={`${LEAF_NORMAL} ${LEAF_STROKE_NORMAL}`} strokeWidth="1" />
        <text x={level2X + 5} y={tnY + 1} textAnchor="middle" className={TEXT_SUBTLE} fontSize="7" fontFamily="system-ui, sans-serif">
          TN: {fmt(data.trueNeg)}
        </text>
        ...
        {highlighted && (
          <>
            {/* Summary box */}
            <rect x={level2X + 50} y={60} width="100" height="80" rx="4" className={LEAF_NORMAL} stroke="#f59e0b" strokeWidth="1" />
            <text x={level2X + 100} y={77} textAnchor="middle" fill="#fbbf24" fontSize="8" fontWeight="700" fontFamily="system-ui, sans-serif">
              All Test Positive
            </text>
            <text x={level2X + 100} y={92} textAnchor="middle" className={TEXT_MID} fontSize="7" fontFamily="system-ui, sans-serif">
            ...
            <line x1={level2X + 60} y1={112} x2={level2X + 140} y2={112} className={LEAF_STROKE_NORMAL} strokeWidth="0.5" />
        ...
        {/* Bottom description */}
        <text x={W / 2} y={H - 38} textAnchor="middle" className={TEXT_MID} fontSize="9" fontFamily="system-ui, sans-serif">
          {round === 1
        ...
        <text x={W / 2} y={H - 24} textAnchor="middle" className={TEXT_SUBTLE} fontSize="8" fontFamily="system-ui, sans-serif">
