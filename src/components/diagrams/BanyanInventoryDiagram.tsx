  ...
  return (
    <div className="my-4">
      <svg viewBox="0 0 550 466" className="w-full max-w-lg mx-auto" role="img" aria-label="Forest inventory plot showing quadrat sampling method with measured trees recording diameter, height, and species">
        ...
        <text x="250" y="44" textAnchor="middle" className="fill-slate-400" fontSize="10">Quadrat sampling: measuring every tree in a defined area</text>
        ...
        {trees.map((tree, i) => (
          <g key={i}>
            ...
            {/* Tag number */}
            <text x={tree.x} y={tree.y - tree.size - 3} textAnchor="middle" className="fill-slate-300" fontSize="7">#{i + 1}</text>
          </g>
        ))}
        ...
        <text x="420" y="328" textAnchor="middle" className="fill-slate-300" fontSize="8">DBH: 60 cm</text>
        <text x="420" y="340" textAnchor="middle" className="fill-slate-300" fontSize="8">Height: 25 m</text>
        <text x="420" y="352" textAnchor="middle" className="fill-slate-300" fontSize="8">Crown: 12 m wide</text>
        ...
        {/* Data table */}
        <rect x="20" y="305" width="310" height="105" rx="6" className="fill-slate-800" />
        <text x="175" y="320" textAnchor="middle" className="fill-slate-300" fontSize="9" fontWeight="bold">Plot Data Summary</text>

        {/* Table header */}
        <text x="35" y="337" className="fill-slate-400" fontSize="7" fontWeight="bold">#</text>
        <text x="55" y="337" className="fill-slate-400" fontSize="7" fontWeight="bold">Species</text>
        <text x="120" y="337" className="fill-slate-400" fontSize="7" fontWeight="bold">DBH</text>
        <text x="155" y="337" className="fill-slate-400" fontSize="7" fontWeight="bold">Height</text>
        <text x="200" y="337" className="fill-slate-400" fontSize="7" fontWeight="bold">BA</text>
        ...
        {trees.slice(0, 4).map((tree, i) => (
          <g key={`row-${i}`}>
            <text x="35" y={353 + i * 12} className="fill-slate-300" fontSize="7">{i + 1}</text>
            <text x="55" y={353 + i * 12} className="fill-green-300" fontSize="7">{tree.sp}</text>
            <text x="120" y={353 + i * 12} className="fill-slate-300" fontSize="7">{tree.d} cm</text>
            <text x="155" y={353 + i * 12} className="fill-slate-300" fontSize="7">{tree.h} m</text>
            <text x="200" y={353 + i * 12} className="fill-slate-300" fontSize="7">{(Math.PI * (tree.d / 200) ** 2).toFixed(3)} m²</text>
          </g>
        ))}
        <text x="35" y={353 + 48} className="fill-slate-500" fontSize="7">... 3 more trees</text>

        {/* Summary stats */}
        <text x="270" y="353" className="fill-amber-300" fontSize="7" fontWeight="bold">7 trees</text>
        <text x="270" y="366" className="fill-slate-300" fontSize="7">Total BA: 0.54 m²</text>
        <text x="270" y="379" className="fill-slate-300" fontSize="7">Per hectare: ~135 m²/ha</text>
        <text x="270" y="392" className="fill-slate-300" fontSize="7">Density: ~1,750 trees/ha</text>
