  ...
  return (
    <div className="w-full max-w-lg mx-auto my-4">
      <svg viewBox={`0 0 ${w} ${h}`} className="w-full" role="img" aria-label="Energy over time diagram showing how a hare rapidly depletes energy through anaerobic sprinting while a tortoise slowly uses energy through aerobic metabolism">
        ...
        <text x={w / 2} y={44} textAnchor="middle" className="fill-gray-500 dark:fill-slate-400" style={{ fontSize: 12 }}>
        ...
        {[0, 5, 10, 15, 20, 25, 30].map(t => (
          <g key={`t${t}`}>
            ...
            <text x={tX(t)} y={bottom + 16} textAnchor="middle" className="fill-gray-500 dark:fill-slate-400" style={{ fontSize: 11 }}>{t}</text>
          </g>
        ))}
        {[0, 25, 50, 75, 100].map(e => (
          <g key={`e${e}`}>
            <line x1={left} y1={eY(e)} x2={right} y2={eY(e)} className="stroke-slate-700" strokeWidth={0.5} />
            <text x={left - 8} y={eY(e) + 4} textAnchor="end" className="fill-gray-500 dark:fill-slate-400" style={{ fontSize: 11 }}>{e}%</text>
          </g>
        ))}
        ...
        <text x={(left + right) / 2} y={bottom + 36} textAnchor="middle" className="fill-gray-600 dark:fill-slate-300" style={{ fontSize: 12 }}>Time (minutes)</text>
        <text x={24} y={(top + bottom) / 2} textAnchor="middle" className="fill-gray-600 dark:fill-slate-300" style={{ fontSize: 12 }} transform={`rotate(-90 24 ${(top + bottom) / 2})`}>Energy remaining (%)</text>
        ...
        <text x={right - 101} y={top + 60} textAnchor="middle" className="fill-gray-600 dark:fill-slate-300" style={{ fontSize: 11 }}>
          Anaerobic: 2 ATP / glucose
        </text>
        <text x={right - 101} y={top + 76} textAnchor="middle" className="fill-gray-600 dark:fill-slate-300" style={{ fontSize: 11 }}>
        ...
        <g>
          ...
          <text x={left + 30} y={bottom - 34} className="fill-gray-700 dark:fill-slate-200" style={{ fontSize: 12 }}>Hare (anaerobic sprint)</text>
          <circle cx={left + 20} cy={bottom - 18} r={5} fill="#22c55e" />
          <text x={left + 30} y={bottom - 14} className="fill-gray-700 dark:fill-slate-200" style={{ fontSize: 12 }}>Tortoise (aerobic cruise)</text>
        </g>
      </svg>
    </div>
  );
