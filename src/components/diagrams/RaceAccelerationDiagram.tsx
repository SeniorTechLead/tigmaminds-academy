      ...
      <svg viewBox={`0 0 ${w} ${h}`} className="w-full" role="img" aria-label="Velocity versus time graph showing a cheetah accelerating rapidly then tiring, compared to a tortoise with slow steady velocity">
        ...
        {[0, 2, 4, 6, 8, 10, 12].map(t => (
          <g key={`t${t}`}>
            ...
            <text x={tX(t)} y={bottom + 16} textAnchor="middle" className="fill-slate-400" style={{ fontSize: 11 }}>{t}s</text>
          </g>
        ))}
        {[0, 5, 10, 15, 20].map(v => (
          <g key={`v${v}`}>
            <line x1={left} y1={vY(v)} x2={right} y2={vY(v)} className="stroke-slate-700" strokeWidth={0.5} />
            <text x={left - 8} y={vY(v) + 4} textAnchor="end" className="fill-slate-400" style={{ fontSize: 11 }}>{v}</text>
          </g>
        ))}
        ...
        <text x={20} y={(top + bottom) / 2} textAnchor="middle" className="fill-slate-300" style={{ fontSize: 12 }} transform={`rotate(-90 20 ${(top + bottom) / 2})`}>Velocity (m/s)</text>
