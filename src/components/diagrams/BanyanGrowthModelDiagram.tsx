  ...
  return (
    <div className="my-4">
      <svg viewBox="0 0 525 440" className="w-full max-w-lg mx-auto" role="img" aria-label="Sigmoid growth model showing tree diameter over time with fast growth when young and slowing with age">
        ...
        <text x="250" y="44" textAnchor="middle" className="fill-slate-400" fontSize="10">Sigmoid curve: fast growth young, slowing with age</text>
        ...
        {/* X axis labels */}
        <text x={(chartL + chartR) / 2} y={chartBot + 30} textAnchor="middle" className="fill-slate-400" fontSize="9">Age (years)</text>
        {[0, 100, 200, 300, 400, 500, 600].map(t => (
          <g key={t}>
            ...
            <text x={xPos(t)} y={chartBot + 16} textAnchor="middle" className="fill-slate-500" fontSize="7">{t}</text>
          </g>
        ))}

        {/* Y axis labels */}
        <text x="30" y={(chartTop + chartBot) / 2} textAnchor="middle" className="fill-slate-400" fontSize="9" transform={`rotate(-90, 30, ${(chartTop + chartBot) / 2})`}>DBH (cm)</text>
        {[0, 50, 100, 150].map(d => (
          <g key={d}>
            <line x1={chartL - 5} y1={yPos(d)} x2={chartL} y2={yPos(d)} className="stroke-slate-500" strokeWidth="1" />
            <text x={chartL - 8} y={yPos(d) + 3} textAnchor="end" className="fill-slate-500" fontSize="7">{d}</text>
        ...
        {/* Phase annotations */}
        {/* Slow start */}
        <rect x={xPos(20)} y={yPos(30)} width="75" height="28" rx="4" className="fill-slate-800" opacity="0.8" />
        <text x={xPos(20) + 37} y={yPos(30) + 12} textAnchor="middle" className="fill-green-300" fontSize="8" fontWeight="bold">Slow start</text>
        <text x={xPos(20) + 37} y={yPos(30) + 23} textAnchor="middle" className="fill-slate-400" fontSize="7">establishing roots</text>
        ...
        <text x={xPos(200) + 55} y={yPos(sigmoid(200)) - 33} textAnchor="middle" className="fill-slate-400" fontSize="7">inflection point</text>
        ...
        <text x={xPos(440) + 35} y={yPos(145) + 23} textAnchor="middle" className="fill-slate-400" fontSize="7">near maximum</text>
        ...
        {/* Equation */}
        <rect x="60" y="330" width="380" height="22" rx="6" className="fill-slate-800" />
        <text x="250" y="345" textAnchor="middle" className="fill-slate-300" fontSize="9">D(t) = K / (1 + e^(-r(t - t₀)))  where K=150, r=0.015, t₀=200</text>
        ...
        <text x="250" y="390" textAnchor="middle" className="fill-slate-400" fontSize="8">A 500-year-old tree took centuries to build its biomass — it cannot be replaced quickly</text>

        <defs>
          <marker id="growthArrow" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto">
            <polygon points="0 0, 8 3, 0 6" className="fill-slate-500" />
          </marker>
        </defs>
      </svg>
    </div>
