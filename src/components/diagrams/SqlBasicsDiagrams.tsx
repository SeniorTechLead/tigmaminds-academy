...
export function SelectWhereDiagram() {
  ...
  return (
    <div className={`${DARK_BG} rounded-xl p-4 max-w-2xl mx-auto my-4`}>
          ...
          <thead>
            <tr className="border-b border-gray-300 dark:border-gray-700">
              {['id', 'name', 'weight', 'park'].map(col => (
                <th key={col} className={`${HEADER} ${filter.cols.includes(col) ? 'text-cyan-700 dark:text-cyan-400' : 'text-gray-600'}`}>{col}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {elephants.map((e, i) => {
              ...
              return (
                <tr key={e.id} className={`border-b border-gray-200 dark:border-gray-800 transition-all duration-500 ${
                  matches ? 'bg-cyan-100 dark:bg-cyan-900/20' : 'opacity-30'
                }`}>
      ...
      <div className="mt-3 font-mono text-xs text-cyan-700 dark:text-cyan-400 bg-gray-100 dark:bg-gray-800 rounded px-3 py-2">
        {filter.label}
      </div>
      ...
      <div className="space-y-1">
        {sorted.map((e, i) => (
          <div key={e.id}
            ...
            <span className={`text-sm font-mono w-16 ${sort.key === 'weight' ? 'text-amber-600 dark:text-amber-400' : 'text-gray-600 dark:text-gray-400'}`}>{e.weight}kg</span>
            <span className={`text-xs ${sort.key === 'name' ? 'text-amber-600 dark:text-amber-400' : 'text-gray-500'}`}>{e.park}</span>
          </div>
        ))}
      </div>

      <div className="mt-3 font-mono text-xs text-amber-600 dark:text-amber-400 bg-gray-100 dark:bg-gray-800 rounded px-3 py-2">
        {sort.label}
      </div>
    </div>
  );
...
export function GroupByDiagram() {
  ...
  return (
    <div className={`${DARK_BG} rounded-xl p-4 max-w-2xl mx-auto my-4`}>
        ...
        <div className="space-y-1">
          {elephants.map(e => (
            <div key={e.id} className="flex items-center gap-3 px-3 py-1.5 rounded bg-gray-100 dark:bg-gray-800">
              ...
              <span className="text-xs text-gray-600 dark:text-gray-400 w-16">{e.weight}kg</span>
              <span className={`text-xs px-2 py-0.5 rounded ${
                e.park === 'Kaziranga' ? 'bg-emerald-100 dark:bg-emerald-900/50 text-emerald-600 dark:text-emerald-400' : 'bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400'
              }`}>{e.park}</span>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-3">
          {groups.map(g => (
            <div key={g.park} className={`rounded-lg border-2 p-3 transition-all duration-500 ${
              g.park === 'Kaziranga' ? 'border-emerald-600 bg-emerald-50 dark:bg-emerald-900/10' : 'border-blue-600 bg-blue-50 dark:bg-blue-900/10'
            }`}>
      ...
      <div className="mt-3 font-mono text-xs text-emerald-600 dark:text-emerald-400 bg-gray-100 dark:bg-gray-800 rounded px-3 py-2">
    ...
    <div className={`${DARK_BG} rounded-xl p-4 max-w-xl mx-auto my-4`}>
      ...
      {!showJoin ? (
        <div className="grid grid-cols-2 gap-4">
          {/* Left table */}
          <div>
            <p className="text-[10px] font-bold text-cyan-700 dark:text-cyan-400 mb-2">elephants</p>
            <table className="w-full">
              ...
              <tbody>
                {leftTable.map(r => (
                  <tr key={r.id} className={`border-b border-gray-200 dark:border-gray-800 cursor-pointer transition-all ${
                    highlight === r.park_id ? 'bg-purple-100 dark:bg-purple-900/30' : 'hover:bg-gray-100 dark:hover:bg-gray-800'
                    ...
                    <td className={`${CELL} ${highlight === r.park_id ? 'text-purple-700 dark:text-purple-400 font-bold' : 'text-gray-600 dark:text-gray-400'}`}>{r.park_id}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {/* Right table */}
          <div>
            <p className="text-[10px] font-bold text-amber-600 dark:text-amber-400 mb-2">parks</p>
            <table className="w-full">
              <thead><tr className="border-b border-gray-300 dark:border-gray-700">
              ...
              <tbody>
                {rightTable.map(r => (
                  <tr key={r.id} className={`border-b border-gray-200 dark:border-gray-800 transition-all ${
                    highlight === r.id ? 'bg-purple-100 dark:bg-purple-900/30' : ''
                  }`}>
        ...
        <div>
          <p className="text-[10px] font-bold text-purple-700 dark:text-purple-400 mb-2">elephants JOIN parks ON park_id = parks.id</p>
          <table className="w-full">
            <thead><tr className="border-b border-gray-300 dark:border-gray-700">
              <th className={`${HEADER} text-cyan-700 dark:text-cyan-400`}>name</th>
              <th className={`${HEADER} text-purple-700 dark:text-purple-400`}>park_id</th>
              <th className={`${HEADER} text-amber-600 dark:text-amber-400`}>park</th>
              <th className={`${HEADER} text-amber-600 dark:text-amber-400`}>state</th>
            </tr></thead>
            <tbody>
              {leftTable.map(l => {
      ...
      <div className="mt-3 font-mono text-xs text-purple-700 dark:text-purple-400 bg-gray-100 dark:bg-gray-800 rounded px-3 py-2">
        {showJoin
  ...
  return (
    <div className={`${DARK_BG} rounded-xl p-4 max-w-2xl mx-auto my-4`}>
      ...
      <div className={`rounded-lg border-2 p-3 transition-all duration-500 ${
        ...
        <p className="font-mono text-xs text-orange-700 dark:text-orange-300">)</p>
      ...
      <table className="w-full">
        ...
        <tbody>
          {rows.map(e => (
            <tr key={e.id} className={`border-b border-gray-200 dark:border-gray-800 transition-all duration-500 ${
              flash?.id === e.id
                ? flash.type === 'insert' ? 'bg-emerald-100 dark:bg-emerald-900/40'
                : flash.type === 'update' ? 'bg-amber-100 dark:bg-amber-900/40'
                : flash.type === 'delete' ? 'bg-red-100 dark:bg-red-900/40 opacity-50 scale-95' : ''
                : ''
            }`}>
