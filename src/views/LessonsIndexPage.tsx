...
export default function LessonsIndexPage() {
  ...
  // Initialise state from URL params (restored on back-navigation)
  const [filterType, setFilterType] = useState<FilterType>((searchParams.get('ft') as FilterType) || 'subject');
  const [selectedSubject, setSelectedSubject] = useState<Subject | null>((searchParams.get('sub') as Subject) || null);
  const [selectedDiscipline, setSelectedDiscipline] = useState<Discipline | null>((searchParams.get('disc') as Discipline) || null);
  const [selectedL2Skill, setSelectedL2Skill] = useState<string | null>(searchParams.get('skill') || null);
  const [selectedL3Tool, setSelectedL3Tool] = useState<string | null>(searchParams.get('tool') || null);
  const [searchQuery, setSearchQuery] = useState(searchParams.get('q') || '');
  const [selectedTradition, setSelectedTradition] = useState<string | null>(searchParams.get('culture') || null);
  const { isStoryComplete, isLevelComplete, getCompletedCount } = useProgress();

  // Sync filter state → URL params (replaceState so it doesn't spam history)
  const syncParams = useCallback(() => {
    const params = new URLSearchParams();
    if (filterType !== 'subject') params.set('ft', filterType);
    if (selectedSubject) params.set('sub', selectedSubject);
    if (selectedDiscipline) params.set('disc', selectedDiscipline);
    if (selectedL2Skill) params.set('skill', selectedL2Skill);
    if (selectedL3Tool) params.set('tool', selectedL3Tool);
    if (searchQuery) params.set('q', searchQuery);
    if (selectedTradition) params.set('culture', selectedTradition);
    const qs = params.toString();
    const url = qs ? `${pathname}?${qs}` : pathname;
    router.replace(url, { scroll: false });
  }, [filterType, selectedSubject, selectedDiscipline, selectedL2Skill, selectedL3Tool, searchQuery, selectedTradition, pathname, router]);
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900 transition-colors">
      ...
      <section className="pt-32 pb-6 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-amber-50 via-white to-orange-50 dark:from-gray-800 dark:via-gray-900 dark:to-gray-800">
        <div className="max-w-7xl mx-auto text-center">
          ...
          {/* Culture/origin filter — subtle secondary row */}
          ...
          {(selectedSubject || selectedDiscipline || selectedTradition) && (
      ...
      <section className="py-12 px-4 sm:px-6 lg:px-8 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-200 dark:border-gray-700">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            <Link href="/plan" className="inline-flex items-center gap-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 px-5 py-3 rounded-xl text-sm font-semibold hover:border-amber-400 hover:text-amber-600 dark:hover:text-amber-400 transition-all">
              <BookOpen className="w-4 h-4" /> Build a Lesson Plan
            </Link>
            <Link href="/library" className="inline-flex items-center gap-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 px-5 py-3 rounded-xl text-sm font-semibold hover:border-amber-400 hover:text-amber-600 dark:hover:text-amber-400 transition-all">
              <Search className="w-4 h-4" /> Reference Library
            </Link>
            <Link href="/playground" className="inline-flex items-center gap-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 px-5 py-3 rounded-xl text-sm font-semibold hover:border-amber-400 hover:text-amber-600 dark:hover:text-amber-400 transition-all">
